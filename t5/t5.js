...
// Funktio, joka käsittelee kartan klikkauksia
function onMapClick(e) {
    popup
        .setLatLng(e.latlng)
        .setContent("Klikkasit kartalle kohdassa " + e.latlng.toString())
        .openOn(map);
}

// Kartan klikkauksen käsittely
...
if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(
        (position) => {
            ...
            // Poistaa mahdollisen edellisen merkkimiehen
            if (window.userMarker) {
                map.removeLayer(window.userMarker);
            }

            // Lisää käyttäjän sijainti kartalle
        ...
        (error) => {
            console.error("Virhe sijainnin saamisessa:", error.message);
        }
    );
} else {
...
//tätä tuun käyttää projektissa. ilmoitun siitä mitä avasin pisteen kohdalla//
/*var popup = L.popup();
...
/*pop up ilmestyy kun painaa  olet tässä merkkiä
...
/* pop up ilmestyy vaan kerran, ei saa auki uudestaan
var popup = L.popup()
